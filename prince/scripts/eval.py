...
def build_model(cfg, dim, device):
    ...
    if model_cfg["model_type"] != "optical_phase_two_stage":
        raise ValueError("prince_only only supports the notebook-style two-stage optical backbone.")
...
def main():
    ap = argparse.ArgumentParser(description="Evaluate the Prince optical-electrical rerank checkpoint on accuracy metrics.")
    ...
    prefix = str(data_cfg["prefix"])
    semantic_npz = cache_dir / f"{prefix}_semantic_k{train_cfg['k']}.npz"
    target_words_npy = cache_dir / f"{prefix}_target_words.npy"
    cand_cache = cache_dir / f"{prefix}_cands_k{train_cfg['cand_k']}.npy"
    ckpt_path = root / (args.ckpt or output_cfg["ckpt"])

    ds_full = OpticalWordDataset(
        str(root / data_cfg["render_dir"]),
        str(cache_dir / f"{prefix}_input_words.npy"),
