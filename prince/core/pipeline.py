...
def build_parser():
    ap = argparse.ArgumentParser(description="Pipeline runner for Prince optical-electrical accuracy evaluation.")
...
def main(argv=None):
    ...
    if model.get("model_type") != "optical_phase_two_stage":
        raise ValueError("prince_only only supports model_type='optical_phase_two_stage'.")
    ...
    semantic_npz = cache_dir / f"{prefix}_semantic_k{k}.npz"
    inputs_int8 = cache_dir / f"{prefix}_inputs_int8.npy"
    cand_cache = cache_dir / f"{prefix}_cands_k{cand_k}.npy"

    input_words_npy = cache_dir / f"{prefix}_input_words.npy"
    target_words_npy = cache_dir / f"{prefix}_target_words.npy"

    if not args.skip_preprocess:
        if reuse_cache and semantic_npz.exists() and inputs_int8.exists() and input_words_npy.exists() and target_words_npy.exists():
            pass
        elif reuse_cache and inputs_int8.exists() and input_words_npy.exists() and target_words_npy.exists():
            run(
                [
                    sys.executable,
