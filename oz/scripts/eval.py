...
def build_model(cfg, dim, device):
    ...
    if model_cfg["model_type"] != "optical_phase_two_stage":
        raise ValueError("oz_only only supports the notebook-style two-stage optical backbone.")
...
def main():
    ap = argparse.ArgumentParser(description="Evaluate the Oz optical-electrical rerank checkpoint on accuracy metrics.")
